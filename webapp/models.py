...
class Post:
    ...
    def get_by_author(db, author_id):
        ...
        result = db.execute(
            f"SELECT * FROM posts WHERE author_id = {author_id} AND published = TRUE ORDER BY created_at DESC"
        )
        if not result.success or not result.rows:
            return []
    ...
    def create(db, title, content, author_id, published=False):
        ...
        next_id = (
            max([p["id"] for p in result.rows], default=0) + 1
            if result.rows
            else 1
        )
...
class User:
    ...
    def get_by_id(db, user_id):
        """Get user by ID"""
        result = db.execute(
            f"SELECT username, full_name FROM users WHERE id = {user_id}"
        )
        if result.rows:
            return result.rows[0]
...
class Comment:
    ...
    def get_by_post(db, post_id):
        ...
        result = db.execute(
            f"SELECT * FROM comments WHERE post_id = {post_id} ORDER BY created_at DESC"
        )
        if not result.rows:
            return []
