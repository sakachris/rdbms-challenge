...
from simpldb.schema import (
    Schema, SchemaManager, Column,
    DataType, ColumnConstraint
)
from simpldb.indexes import IndexManager
from simpldb.parser import (
    Query, QueryType, Operator,
    CreateTableQuery, DropTableQuery,
    CreateIndexQuery, DropIndexQuery,
    InsertQuery, SelectQuery, UpdateQuery, DeleteQuery
)
...
class QueryResult:
    ...
    def __repr__(self):
        if self.rows:
            return f"QueryResult(success={self.success}, rows={len(self.rows)}, affected={self.rows_affected})"
        return f"QueryResult(success={self.success}, message='{self.message}', affected={self.rows_affected})"
...
class QueryExecutor:
    ...
    def execute(self, query: Query) -> QueryResult:
        ...
        try:
            if query.query_type == QueryType.CREATE_TABLE:
                ...
                return QueryResult(False, f"Unsupported query type: {query.query_type}")
    ...
    def _execute_create_table(self, query: CreateTableQuery) -> QueryResult:
        ...
        if self.schema_manager.table_exists(query.table_name):
            return QueryResult(False, f"Table '{query.table_name}' already exists")
        ...
        for col_def in query.columns:
            ...
            try:
                ...
                return QueryResult(False, f"Unknown data type: {col_def.data_type}")
            ...
            for constraint_str in col_def.constraints:
                try:
                    ...
                    return QueryResult(False, f"Unknown constraint: {constraint_str}")
        ...
        return QueryResult(True, f"Table '{query.table_name}' created successfully")
    
    def _execute_drop_table(self, query: DropTableQuery) -> QueryResult:
        """Execute DROP TABLE"""
        if not self.schema_manager.table_exists(query.table_name):
            return QueryResult(False, f"Table '{query.table_name}' does not exist")
        ...
        return QueryResult(True, f"Table '{query.table_name}' dropped successfully")
    ...
    def _execute_create_index(self, query: CreateIndexQuery) -> QueryResult:
        ...
        if not schema.get_column(query.column_name):
            return QueryResult(False, f"Column '{query.column_name}' does not exist")
        
        if self.index_manager.has_index(query.table_name, query.column_name):
            return QueryResult(False, f"Index on '{query.table_name}.{query.column_name}' already exists")
        ...
        self.index_manager.rebuild_index(query.table_name, query.column_name, rows)
        
        return QueryResult(True, f"Index '{query.index_name}' created successfully")
    
    def _execute_drop_index(self, query: DropIndexQuery) -> QueryResult:
        """Execute DROP INDEX"""
        if not self.index_manager.has_index(query.table_name, query.index_name):
            return QueryResult(False, f"Index '{query.index_name}' does not exist")
        
        self.index_manager.drop_index(query.table_name, query.index_name)
        return QueryResult(True, f"Index '{query.index_name}' dropped successfully")
    ...
    def _execute_insert(self, query: InsertQuery) -> QueryResult:
        ...
        if not is_valid:
            return QueryResult(False, f"Validation failed: {', '.join(errors)}")
        
        # Convert types
        converted = schema.convert_row(row_data)
        
        # Check unique constraints via indexes
        existing = [r.to_dict() for r in self.storage.get_table(query.table_name).select_all()]
        ...
        return QueryResult(True, f"1 row inserted (ID: {row_id})", rows_affected=1)
    ...
    def _execute_select(self, query: SelectQuery) -> QueryResult:
        ...
        if query.where and len(query.where) > 0:
            ...
            if candidate_row_ids is not None:
                # Use indexed lookup
                rows = [table.select_by_id(row_id) for row_id in candidate_row_ids]
        ...
        # Apply WHERE filter
        if query.where:
            rows = [row for row in rows if self._evaluate_conditions(row, query.where)]
        ...
        return QueryResult(True, f"{len(result_rows)} rows selected", rows=result_rows)
    ...
    def _execute_update(self, query: UpdateQuery) -> QueryResult:
        ...
        if query.where:
            rows_to_update = [row for row in rows_to_update 
        ...
        return QueryResult(True, f"{updated_count} rows updated", rows_affected=updated_count)
    ...
    def _execute_delete(self, query: DeleteQuery) -> QueryResult:
        ...
        if query.where:
            rows_to_delete = [row for row in rows_to_delete 
                             if self._evaluate_conditions(row, query.where)]
        ...
        for row in rows_to_delete:
            # Delete from indexes
            self.index_manager.delete_from_indexes(query.table_name, row.row_id, row.data)
            
            # Delete from storage
            table.delete_by_id(row.row_id)
            deleted_count += 1
        
        return QueryResult(True, f"{deleted_count} rows deleted", rows_affected=deleted_count)
    
    def _get_candidate_rows_with_index(self, table_name: str, 
                                      conditions: List) -> Optional[List[int]]:
        """Try to use an index to get candidate rows"""
        for condition in conditions:
            if condition.operator == Operator.EQ:
                # Check if we have an index on this column
                index = self.index_manager.get_index(table_name, condition.column)
                if index:
                    return index.search(condition.value)
            elif condition.operator in (Operator.GT, Operator.GE, Operator.LT, Operator.LE):
                # Range query
                index = self.index_manager.get_index(table_name, condition.column)
                if index:
                    if condition.operator == Operator.GT:
                        return index.range_search(condition.value, None, include_start=False)
                    elif condition.operator == Operator.GE:
                        return index.range_search(condition.value, None, include_start=True)
                    elif condition.operator == Operator.LT:
                        return index.range_search(None, condition.value, include_end=False)
                    elif condition.operator == Operator.LE:
                        return index.range_search(None, condition.value, include_end=True)
    ...
    def _evaluate_conditions(self, row, conditions: List) -> bool:
        ...
        for condition in conditions:
            value = row.data.get(condition.column) if hasattr(row, 'data') else row.get(condition.column)
    ...
    def _execute_joins(self, query: SelectQuery, left_rows: List, left_schema: Schema) -> List:
        ...
        for join in query.joins:
            ...
            for left_row in left_rows:
                left_val = left_row.data.get(left_col) if hasattr(left_row, 'data') else left_row.get(left_col)
                matched = False
                ...
                for right_row in right_rows:
                    ...
                    if left_val == right_val:
                        ...
                        if hasattr(left_row, 'data'):
                            combined.update({f"{query.table_name}.{k}": v for k, v in left_row.data.items()})
                        else:
                            combined.update({f"{query.table_name}.{k}": v for k, v in left_row.items()})
                        combined.update({f"{join.table}.{k}": v for k, v in right_row.data.items()})
...
if __name__ == "__main__":
    ...
    # Test queries
    test_sqls = [
        "CREATE TABLE users (id INTEGER PRIMARY KEY, name VARCHAR(100) NOT NULL, age INTEGER, email VARCHAR(100) UNIQUE)",
