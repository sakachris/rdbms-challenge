...
class SimplDBREPL:
    ...
    def _completer(self, text, state):
        ...
        # Add SQL keywords
        options.extend(
            [kw for kw in sql_keywords if kw.startswith(text.upper())]
        )

        # Add meta commands
        options.extend(
            [cmd for cmd in meta_commands if cmd.startswith(text.lower())]
        )
        ...
        try:
            ...
            options.extend(
                [tbl for tbl in tables if tbl.startswith(text.lower())]
            )
        except Exception:
            pass
        ...
        try:
            ...
            if result.success:
                if result.rows:
                    ...
                    print(
                        f"\n{len(result.rows)} row(s) returned in "
                        f"{elapsed:.3f}s"
                    )
                else:
                    ...
                    if result.rows_affected > 0:
                        print(
                            f"{result.rows_affected} row(s) affected in "
                            f"{elapsed:.3f}s"
                        )
                    else:
    ...
    def display_results(self, rows):
        ...
        # (in case of joins with different columns)
        headers = []
        seen = set()
        for row in rows:
            for key in row.keys():
...
"""
    ...
    def _show_table_schema(self, table_name: str):
        ...
        if "columns" in info:
            ...
            for col in info["columns"]:
                ...
                print(
                    f"  {col['name']:20} {col['type']}{length:10} "
    ...
    def describe_table(self, args):
        ...
        if "columns" in info:
            ...
            for col in info["columns"]:
                ...
                default = (
                    col["default"] if col.get("default") is not None else "-"
                )
            ...
            print(
                tabulate(
                    col_data,
                    headers=["Name", "Type", "Constraints", "Default"],
                    tablefmt="psql",
                )
            )
    ...
    def import_schema(self, args):
        ...
        try:
            ...
            print(
                f"Imported {success_count}/{len(results)} tables successfully"
            )
        except Exception as e:
