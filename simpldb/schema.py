...
class Column:
    ...
    def __init__(
        ...
        if data_type == DataType.VARCHAR and max_length is None:
            raise ValueError(
                f"VARCHAR column '{name}' must specify max_length"
            )
    ...
    def is_unique(self) -> bool:
        return (
            ColumnConstraint.UNIQUE in self.constraints
            or self.is_primary_key()
        )
        ...
        if self.data_type == DataType.VARCHAR:
            if len(str(converted_value)) > self.max_length:
                return (
                    False,
                    (
                        f"Value for '{self.name}' exceeds max length of "
                        f"{self.max_length}"
                    ),
                )
    ...
    def convert_value(self, value: Any) -> Any:
        ...
        if self.data_type == DataType.INTEGER:
            ...
            or self.data_type == DataType.TEXT
        ):
            return str(value)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Column":
        ...
        return cls(
            ...
            constraints=[
                ColumnConstraint(c) for c in data.get("constraints", [])
            ],
    ...
    def __repr__(self):
        ...
        return (
            f"{self.name} {self.data_type.value}{length_str} {constraints_str}"
        ).strip()
...
class Schema:
    ...
    def _validate_schema(self):
        ...
        primary_keys = [
            col for col in self.columns.values() if col.is_primary_key()
        ]
    ...
    def check_unique_constraint(
        ...
        for row in existing_rows:
            ...
            if row_value == value:
                constraint_type = (
                    "PRIMARY KEY" if column.is_primary_key() else "UNIQUE"
                )
                return (
                    False,
                    (
                        f"{constraint_type} violation: "
                        f"'{column_name}' value '{value}' already exists"
                    ),
                )
...
if __name__ == "__main__":
    ...
    columns = [
        Column(
            "id", DataType.INTEGER, constraints=[ColumnConstraint.PRIMARY_KEY]
        ),
        Column(
            "username",
            DataType.VARCHAR,
    ...
    print(
        f"   Types: "
        f"id={type(converted['id'])}, "
        f"balance={type(converted['balance'])}, "
        f"is_active={type(converted['is_active'])}"
    )
