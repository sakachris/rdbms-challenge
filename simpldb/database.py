    ...
    def unregister_table(self, table_name: str):
        ...
        with self.lock:
            ...
            self.catalog['indexes'] = {
                k: v for k, v in self.catalog['indexes'].items()
                if not k.startswith(f"{table_name}.")
            }
            self._save_catalog()
    
    def register_index(self, table_name: str, column_name: str, 
                      index_name: str, unique: bool):
    ...
    def update_table_stats(self, table_name: str, row_count: int):
        ...
        with self.lock:
            if table_name in self.catalog['tables']:
                ...
                self.catalog['tables'][table_name]['last_modified'] = datetime.now().isoformat()
                self._save_catalog()
        ...
        for table_name, table_info in self.catalog.catalog['tables'].items():
            if 'schema' in table_info:
                try:
                    ...
                    print(f"Warning: Could not load schema for {table_name}: {e}")
...
if __name__ == "__main__":
    ...
    print("\n3. Inserting data...")
    inserts = [
        "INSERT INTO users (id, username, email, age, balance) VALUES (1, 'alice', 'alice@example.com', 30, 1000.0)",
        "INSERT INTO users (id, username, email, age, balance) VALUES (2, 'bob', 'bob@example.com', 25, 500.0)",
        "INSERT INTO users (id, username, email, age, balance) VALUES (3, 'charlie', 'charlie@example.com', 35, 1500.0)"
    ]
    ...
    # Update data
    print("\n6. Updating data...")
    result = db.execute("UPDATE users SET balance = 1100.0 WHERE username = 'alice'")
    print(f"   {result.message}")
    ...
    print(f"   ✓ Schema exported to schema_export.json")
