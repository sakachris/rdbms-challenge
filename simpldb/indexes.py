...
class BTreeNode:
    ...
    def __init__(self, is_leaf: bool = True):
        ...
        self.values: List[
            List[int]
        ] = []  # Each key can map to multiple row IDs
...
class Index:
                ...
                if existing and row_id not in existing:
                    return (
                        ...
                        (
                            ...
                            f"{self.column_name}: '{key}' already exists"
                        ),
                    )
    ...
    def update(
        self, old_key: Any, new_key: Any, row_id: int
            ...
            if self.unique:
                ...
                if existing and row_id not in existing:
                    ...
                    return (
                        ...
                        (
                            f"Unique constraint violation on "
                            f"{self.column_name}: '{new_key}' already exists"
                        ),
                    )
...
class IndexManager:
    ...
    def _save_indexes(self, table_name: str):
        ...
        data = {
            ...
            "indexes": [
                idx.to_dict() for idx in self.indexes[table_name].values()
            ],
        }
    ...
    def drop_index(self, table_name: str, column_name: str):
        ...
        with self.lock:
            if (
                table_name in self.indexes
                and column_name in self.indexes[table_name]
            ):
    ...
    def rebuild_index(
        self, table_name: str, column_name: str, rows: List[Dict]
    ) -> bool:
        """
        ...
        with self.lock:
            ...
            for row in rows:
                ...
                if value is not None and row_id is not None:
                    ...
                    if not success:
                        print(
                            f"Warning: Could not index row {row_id}: {error}"
                        )
    ...
    def insert_into_indexes(
        self, table_name: str, row_id: int, row_data: Dict[str, Any]
    ) -> Tuple[bool, Optional[str]]:
            ...
            for column_name, index in self.indexes[table_name].items():
                ...
                if old_value != new_value:
                    if new_value is not None:
                        success, error = index.update(
                            old_value, new_value, row_id
                        )
                        if not success:
                            # Rollback
    ...
    def get_all_stats(self) -> Dict[str, List[Dict]]:
        ...
        for table_name, table_indexes in self.indexes.items():
            stats[table_name] = [
                idx.get_stats() for idx in table_indexes.values()
            ]
        return stats
...
if __name__ == "__main__":
    ...
    success, error = manager.insert_into_indexes(
        "users", 5, {"id": 5, "email": "alice@example.com", "age": 28}
    )
    ...
    manager.delete_from_indexes(
        "users", 2, {"id": 2, "email": "bob@example.com", "age": 25}
    )
